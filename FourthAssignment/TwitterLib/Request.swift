...
private var twitterAccount: TWTRSession?
...
public class Request: NSObject
{
    ...
    public func fetchTweets(_ handler: @escaping ([TweetStruct]) -> Void) {
        fetch { results in
            ...
            if let dictionary = results as? NSDictionary {
                if let tweets = dictionary[TwitterKey.tweets] as? NSArray {
                    tweetArray = tweets
                } else if let tweet = TweetStruct(data: dictionary) {
                    tweets = [tweet]
                }
            ...
            if tweetArray != nil {
                for tweetData in tweetArray! {
                    if let tweet = TweetStruct(data: tweetData as? NSDictionary) {
                        tweets.append(tweet)
                    }
                }
            }
    ...
    func performTwitterRequest(_ method: String, handler: @escaping (PropertyList?) -> Void) {
        performTwitterTWRequest(handler: handler)
    }
    ...
    func performTwitterTWRequest(handler: @escaping (PropertyList?) -> Void) {
        if let twitterAccountId = Twitter.sharedInstance().sessionStore.session()?.userID {
            ...
            Twitter.sharedInstance().logIn(completion: { (session, error) in
                if let sess = session {
                    twitterAccount = sess
                    self.performTwitterTWRequest(handler: handler)
                } else {
